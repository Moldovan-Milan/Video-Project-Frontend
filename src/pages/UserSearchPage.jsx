  ...
  const handleInputChange = (type, value) => {
    ...
    debounceTimeout.current = setTimeout(() => {
      ...
      setDebouncedUsername(
        type === "username" ? value.trim() : username.trim()
      );
    }, DEBOUNCE_DELAY);
  };
      ...
      try {
        ...
        if (debouncedUserId !== "") {
          ...
          const response = await axios.get(
            `/api/user/search/${debouncedUsername}?pageSize=${pageSize}&pageNumber=${pageNumber}`
          );
      ...
      <div className="user-list-container flex flex-col gap-4">
        {users.map((user, index) => {
          if (
            ...
            return (
              <Link key={user.id} to={`/admin/edit-user-roles/${user.id}`}>
                <UserItem user={user} />
              </Link>
            );
          }
