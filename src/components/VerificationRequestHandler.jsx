...
const VerificationRequestHandler = ({user}) => {

  const handleAccept = async () => {
        try{
            const response = await axios.post(`api/Admin/verify-user/${user.id}`, {},
                {
  ...
  const handleDecline = async () => {
    try{
        const response = await axios.post(`api/Admin/decline-verification/${user.id}`, {},
            {
            withCredentials: true
        })
        if(response.status === 200){
